...
export default function ResponsiveLayout({ children, className }: ResponsiveLayoutProps) {
  ...
  return (
    <div className={layoutClasses} data-device-type={device.isMobile ? 'mobile' : device.isTablet ? 'tablet' : 'desktop'}>
      ...
      {/* Add device info overlay for development */}
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed bottom-2 left-2 bg-black/80 text-white text-xs p-2 rounded z-50 pointer-events-none">
          {device.screenWidth}x{device.screenHeight} | {device.osName} | {device.browserName}
        </div>
      )}
    </div>
  );
