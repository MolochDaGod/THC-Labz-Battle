...
// Agent AI wallet configuration
const AGENT_AI_WALLET = "ErSGeWkLuKqmW2MNrcFWPsYryNPXDW224GmgNBf8ZT65";
...
export async function processLeaderboardRewards() {
  ...
  try {
    ...
    // Let Grench AI decide if rewards should be processed
    ...
    console.log("📊 AI Recommendations:", aiDecision.recommendations);
    ...
    const rewardedIds = rewardedPlayers.map(p => topPlayers.find(tp => tp.name === p.name)?.id).filter(Boolean);
