...
const THC_SWAP_FEE_WALLET = '98jzgFFkPhrw9sfr5YyttTpCBiJyid6tzxxJjXrj7xXK'; // 3% fee recipient
...
export async function executeTokenSwap(req: Request, res: Response) {
  try {
    ...
    if (direction === 'budz-to-thc') {
      ...
      // BUDZ → GBUX: Stable 1:10 ratio (1 BUDZ = 10 GBUX)
      console.log('🔥 Processing BUDZ → GBUX swap via AI Agent...');
      
      const gbuxAmount = amount * 10; // 1 BUDZ = 10 GBUX stable rate
      
      console.log(`💱 Stable swap: ${amount} BUDZ → ${gbuxAmount} GBUX (1:10 rate)`);
      ...
      // GBUX → BUDZ: Stable 10:1 ratio (10 GBUX = 1 BUDZ)
      console.log('🔥 Processing GBUX → BUDZ swap via AI Agent...');
      
      const budzAmount = Math.floor(amount / 10); // 10 GBUX = 1 BUDZ stable rate
      
      console.log(`💱 Stable swap: ${amount} GBUX → ${budzAmount} BUDZ (10:1 rate)`);
