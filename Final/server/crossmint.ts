...
class CrossmintWalletService {
  ...
  async createWallet(userId: string): Promise<CrossmintWalletResponse> {
    // Web3 production requires proper API credentials
    if (!this.apiKey || !this.projectId) {
      console.error(`❌ Cannot create wallet for user ${userId} - Crossmint API credentials required for Web3 production`);
      throw new Error('Crossmint API credentials required for wallet creation - no demo wallets in production');
    }
    ...
    try {
      ...
      console.error(`❌ Crossmint wallet creation failed for Web3 production: ${error.message}`);
      throw new Error(`Web3 wallet creation failed - ${error.message}`);
    }
  }
