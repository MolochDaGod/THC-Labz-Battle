...
import { getHeliusRPCUrl, HELIUS_CONFIG, FALLBACK_RPC_URLS, heliusRateLimiter } from './helius-rate-limiter';
...
/**
 * Helius API configuration with daily rate limiting
 */
const HELIUS_API_URL = `${HELIUS_CONFIG.API_BASE_URL}/token-metadata?api-key=${HELIUS_CONFIG.API_KEY}`;
const HELIUS_PRICE_API = `${HELIUS_CONFIG.API_BASE_URL}/tokens/price-history?api-key=${HELIUS_CONFIG.API_KEY}`;

// Helper function to get primary RPC URL with rate limiting
function getPrimaryRPCUrl(): string {
  const heliusUrl = getHeliusRPCUrl();
  return heliusUrl || FALLBACK_RPC_URLS[0];
}
...
const THC_SWAP_FEE_WALLET = 'ErSGeWkLuKqmW2MNrcFWPsYryNPXDW224GmgNBf8ZT65'; // AI Agent wallet for fee collection
...
export async function executeTokenSwap(req: Request, res: Response) {
  try {
    ...
    // Calculate expected output amount for validation with correct THC GROWERZ conversion rates
    ...
    else if (direction === 'budz-to-thc' || direction === 'gbux-to-thc') expectedAmount = Math.floor(Math.floor(amount / 81.3) * 0.97); // THC conversion with 3% AI fee
    else if (direction === 'thc-to-budz') expectedAmount = Math.floor(amount * 81.3 * 0.95); // THC to BUDZ with 5% processing fee
    ...
    if (direction === 'budz-to-thc') {
      // BUDZ → THC LABZ: Calculate based on USD equivalent with 3% AI Agent fee
      // THC GROWERZ = $0.001 USD per token (fixed)
      // BUDZ = $0.0000123 USD per token (current price)
      // Conversion rate: 1 THC = 81.3 BUDZ (0.001 / 0.0000123)
      
      console.log('🔥 AI Agent processing BUDZ → THC LABZ swap...');
      
      const budzPriceUSD = 0.0000123; // Current BUDZ price
      const thcPriceUSD = 0.001; // THC GROWERZ fixed price
      const conversionRate = thcPriceUSD / budzPriceUSD; // ~81.3 BUDZ per THC
      const thcAmount = Math.floor(amount / conversionRate);
      const thcAmountWithFee = Math.floor(thcAmount * 0.97); // Apply 3% AI fee
      
      console.log(`💱 AI Agent swap: ${amount} BUDZ → ${thcAmount} THC → ${thcAmountWithFee} THC after 3% fee`);
      
      // Burn BUDZ tokens to remove from circulation
      ...
      // AI Agent transfers THC LABZ to user with 3% fee applied
      ...
      console.log('✅ AI Agent completed THC LABZ transfer with fee applied');
      
    } else if (direction === 'gbux-to-thc') {
      // GBUX → THC LABZ: Calculate based on USD equivalent with 3% AI Agent fee
      // THC GROWERZ = $0.001 USD per token (fixed)
      // GBUX = $0.0000123 USD per token (current price)
      // Conversion rate: 1 THC = 81.3 GBUX (0.001 / 0.0000123)
      
      console.log('🔥 AI Agent processing GBUX → THC LABZ swap...');
      ...
      console.log(`💱 AI Agent swap: ${amount} GBUX → ${thcAmount} THC → ${thcAmountWithFee} THC after 3% fee`);
      
      // Transfer GBUX to AI Agent wallet (accumulates for liquidity)
...
async function getRealTokenBalances(walletAddress: string) {
  try {
    ...
    const primaryRpc = getPrimaryRPCUrl();
    console.log(`🌐 Using primary RPC URL: ${primaryRpc}`);
    
    // Try primary RPC first, then fallback
    let response = await fetch(primaryRpc, {
      method: 'POST',
      headers: {
    ...
    if (data.error && data.error.code === -32401) {
      console.log(`🔄 Primary RPC failed, trying fallback: ${FALLBACK_RPC_URLS[0]}`);
      response = await fetch(FALLBACK_RPC_URLS[0], {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    // Get SOL balance
    let solResponse = await fetch(primaryRpc, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (solData.error && solData.error.code === -32401) {
      ...
      solResponse = await fetch(FALLBACK_RPC_URLS[0], {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
