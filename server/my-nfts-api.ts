...
/**
 * Convert IPFS / Arweave protocol URIs to loadable HTTPS URLs
 */
function resolveImageUrl(uri: string | undefined | null): string {
  if (!uri) return '';
...
/**
 * Fetch the actual image for an NFT mint by:
 *  1. Helius DAS API (offchain/onchain metadata)
 *  2. getAsset RPC (Metaplex DAS)
 * Returns an HTTPS image URL, or empty string if nothing found.
 */
async function fetchMetaplexImage(mintAddress: string): Promise<string> {
  try {
    // ── Strategy 1: Helius token-metadata endpoint ────────────────────────
    if (process.env.HELIUS_PROJECT_ID) {
      try {
        const res = await fetch(
          `https://api.helius.xyz/v0/token-metadata?api-key=${process.env.HELIUS_PROJECT_ID}`,
          {
            method: 'POST',
        ...
        if (res.ok) {
          ...
          const meta = data?.[0];
          if (meta) {
            const offChainImage = meta.offChainData?.image;
            if (offChainImage) return resolveImageUrl(offChainImage);

            const metadataUri = meta.onChainMetadata?.metadata?.uri;
            if (metadataUri) {
              const resolved = resolveImageUrl(metadataUri);
              const jsonRes = await fetch(resolved, { signal: AbortSignal.timeout(5000) });
              if (jsonRes.ok) {
                const json = await jsonRes.json();
                if (json.image) return resolveImageUrl(json.image);
              }
            }
          }
        }
    ...
    // ── Strategy 2: getAsset (Metaplex DAS) via Helius or public RPC ─────
    ...
    try {
      ...
      if (assetRes.ok) {
        ...
        // DAS response has content.links.image or content.json_uri
        const directImage = assetData?.result?.content?.links?.image;
        if (directImage) return resolveImageUrl(directImage);

        const jsonUri = assetData?.result?.content?.json_uri;
        if (jsonUri) {
          const resolved = resolveImageUrl(jsonUri);
          const jsonRes = await fetch(resolved, { signal: AbortSignal.timeout(5000) });
          if (jsonRes.ok) {
            const json = await jsonRes.json();
            if (json.image) return resolveImageUrl(json.image);
          ...
          if (collectionData.success && collectionData.nfts) {
            ...
            for (const mintAddress of userNFTMints) {
              ...
              if (matchingNFT) {
                ...
                // Resolve image URL — convert IPFS/Arweave or fetch on-chain metadata
                let image = resolveImageUrl(matchingNFT.image);
                if (!image) {
                  console.log(`🖼️ MY NFTS: No image for ${matchingNFT.name}, fetching on-chain metadata...`);
                  image = await fetchMetaplexImage(mintAddress);
                  if (image) console.log(`✅ MY NFTS: Resolved image for ${matchingNFT.name}`);
                }
                userNFTs.push({ ...matchingNFT, image });
              }
            }
...
async function scanWalletForNFTs(walletAddress: string): Promise<string[]> {
  const heliusKeyUrl = process.env.HELIUS_API_KEY
        ...
        try {
          ...
          if (heliusData.length > 0) {
            ...
            if (metadata.onChainMetadata?.metadata?.name?.includes('Growerz') || 
              ...
              const rawImage = metadata.offChainData?.image
                || metadata.onChainMetadata?.metadata?.image
                || '';
              const nft = {
                mint: mintAddress,
                name: metadata.onChainMetadata.metadata.name || `THC GROWERZ NFT`,
                image: resolveImageUrl(rawImage) || await fetchMetaplexImage(mintAddress),
        ...
        if (magicEdenResponse.ok) {
          ...
          if (meData.collection === 'thc_labz_growerz' || 
            ...
            const nft = {
              ...
              image: resolveImageUrl(meData.image) || await fetchMetaplexImage(mintAddress),
