...
class CrossmintWalletService {
  ...
  constructor() {
    ...
    if (!this.apiKey || !this.projectId) {
      console.warn('⚠️ Crossmint API credentials not fully configured - using fallback wallet creation');
    } else {
      console.log('✅ Crossmint API credentials configured');
    }
  }
  ...
  async createWallet(userId: string): Promise<CrossmintWalletResponse> {
    ...
    if (!this.apiKey || !this.projectId) {
      console.warn(`⚠️ Crossmint API credentials not configured - creating fallback wallet for ${userId}`);
      
      // Return a simulated wallet for development/testing
      const fallbackWallet: CrossmintWalletResponse = {
        type: 'solana-mpc-wallet',
        address: `DEV_${userId.slice(0, 8)}_${Date.now().toString(36)}`,
        linkedUser: `thc_clash_${userId.slice(0, 8)}_${Date.now()}`,
        createdAt: new Date().toISOString()
      };
      
      console.log(`🎮 Fallback wallet created for development: ${fallbackWallet.address}`);
    ...
    try {
      ...
      console.error(`❌ Crossmint wallet creation failed: ${error.message}`);
      
      // Return fallback wallet on API failure
      const fallbackWallet: CrossmintWalletResponse = {
        type: 'solana-mpc-wallet-fallback',
        address: `API_FAIL_${userId.slice(0, 8)}_${Date.now().toString(36)}`,
        linkedUser: `thc_clash_${userId.slice(0, 8)}_${Date.now()}`,
        createdAt: new Date().toISOString()
      };
      
      console.log(`🎮 Fallback wallet created due to API error: ${fallbackWallet.address}`);
      return fallbackWallet;
    }
  }
  ...
  /**
   * Mint a compressed NFT (cNFT) for a game card using the THC CLASH collection
  ...
  async mintCardNFT(
  ...
  ): Promise<{ success: boolean; mintId?: string; txHash?: string; error?: string }> {
    const COLLECTION_ID = '2397b172-1803-403f-9d30-4dc553776c58';

    const metadata = {
      name: cardData.name,
      image: cardData.image,
      description: cardData.description || `THC CLASH card: ${cardData.name} — a ${cardData.rarity} ${cardData.class} unit`,
      attributes: [
        { trait_type: 'Rarity', value: cardData.rarity },
        { trait_type: 'Class', value: cardData.class },
        { trait_type: 'Attack', value: cardData.attack.toString() },
        { trait_type: 'Health', value: cardData.health.toString() },
        { trait_type: 'Cost', value: cardData.cost.toString() },
        { trait_type: 'Card ID', value: cardData.id },
        { trait_type: 'Game', value: 'THC CLASH' },
      ]
    };

    console.log(`🎴 Minting cNFT for card "${cardData.name}" to wallet ${recipientWallet}`);
    ...
    try {
      ...
      console.log(`✅ cNFT mint initiated: ${result.id || result.mintId}`);
      return {
        success: true,
        mintId: result.id || result.mintId,
        txHash: result.txHash || result.onChain?.txId
      };
    } catch (error: any) {
      console.error(`❌ cNFT mint failed:`, error.message);
      return { success: false, error: error.message };
    }
  }

  /**
   * Get NFT mint status by mint ID
   */
  async getMintStatus(mintId: string): Promise<any> {
    return await this.makeRequest(`/collections/2397b172-1803-403f-9d30-4dc553776c58/nfts/${mintId}`);
  }

  /**
   * Transfer a compressed NFT (cNFT) from a Crossmint-managed wallet to a destination wallet.
   * Used for auction settlement when a card has an on-chain NFT mint address.
   *   fromWalletId  — Crossmint wallet ID (agent/seller's Crossmint wallet)
   *   toAddress     — destination Solana wallet (buyer)
   *   nftMint       — the compressed NFT mint/asset address on Solana
   */
  async transferNft(
    fromWalletId: string,
    toAddress: string,
    ...
    console.log(`🖼️ Transferring cNFT ${nftMint} from wallet ${fromWalletId} → ${toAddress}`);
    const response = await this.makeRequest('/wallets/transfers', 'POST', {
      fromWalletId,
      recipient: `solana:${toAddress}`,
      requestId: `nft_transfer_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      params: {
        nftId:      nftMint,
        blockchain: 'solana',
      },
    });
    console.log(`✅ cNFT transfer initiated: ${response.id} — status: ${response.status}`);
    return response;
  }
}
