...
export const detectWeb3Browser = (): BrowserInfo => {
  ...
  if (isPhantom) {
    return {
      ...
      optimizations: ['auto-connect', 'enhanced-security', 'native-solana']
    };
  }
...
export const detectBrowserSpecificWallets = () => {
  ...
  // Brave browser Solana wallet support
  if (browserInfo.name === 'Brave Browser' && window.solana) {
    console.log('✅ Brave Solana wallet support detected');
  }

  // Opera browser Solana wallet support  
  if (browserInfo.name === 'Opera Browser' && window.solana) {
    console.log('✅ Opera Solana wallet support detected');
  }

  // Enhanced Solflare detection with multiple paths
  if (window.solflare || 
      (window as any).solflare || 
      (window.solana as any)?.isSolflare ||
      (window as any).solflareWallet) {
    wallets.push('Solflare');
    console.log('✅ Solflare detected via multiple detection paths');
  }
  ...
  // Enhanced Magic Eden detection with multiple paths
  if (window.magicEden?.solana || 
    ...
    console.log('✅ Magic Eden detected via multiple detection paths');
  }
