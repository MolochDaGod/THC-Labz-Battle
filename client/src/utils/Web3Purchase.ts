...
// BUDZ = on-chain BUDZ token transfer
// SOL  = native SOL transfer
// GAME_TOKEN = THC LABZ token transfer
...
export async function fetchAllTokenPrices(): Promise<TokenPrices> {
  try {
    const [batchResp, solPrice] = await Promise.all([
      fetch('/api/token-prices/batch', { method: 'POST', headers: { 'Content-Type': 'application/json' } }),
      fetchSolPrice(),
    ]);
    if (batchResp.ok) {
      ...
      return {
        sol: solPrice,
        gbux: d.budz || d.gbux || 0.0000123, // GBUX = BUDZ price
        budz: d.budz || d.gbux || 0.0000123,
...
export async function getPackQuote(
  ...
  if (paymentToken === 'SOL') {
    ...
    // GBUX uses in-game balance path, shouldn't reach here for on-chain
    tokenMint = BUDZ_MINT;
  }
...
export async function executePurchaseAndOpenPack(
  ...
  try {
    ...
    // GBUX = in-game BUDZ balance path (no on-chain tx)
    if (paymentToken === 'GBUX') {
      onStatus?.('Deducting GBUX...');
    ...
    const usdAmt = PACK_USD_PRICES[packType] ?? 0.10;
    const tokenLabel = paymentToken === 'SOL' ? 'SOL' : paymentToken === 'BUDZ' ? 'BUDZ' : 'THC';
    onStatus?.(`Building tx: ${usdAmt.toFixed(2)} USD in ${tokenLabel}...`);
    ...
    onStatus?.('AI agent verifying payment...');
    const resp = await fetch('/api/card-shop/verify-tx', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ signature, walletAddress, packType, paymentToken, uiAmount: quote.uiAmount }),
    });
    const data = await resp.json();
