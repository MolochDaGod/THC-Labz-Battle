import { pgTable, text, serial, integer, bigint, boolean, varchar, timestamp, numeric } from "drizzle-orm/pg-core";
...
export const users = pgTable("users", {
  ...
  thcBalance: bigint("thc_balance", { mode: 'number' }).default(0).notNull(),
  // Social connections
...
// Game Types and NFT Bonuses for Universal Game Integration
export const gameTypes = pgTable("game_types", {
  id: serial("id").primaryKey(),
  name: varchar("name").notNull(),
  category: varchar("category").notNull(), // 'rpg', 'strategy', 'shooter', etc.
  description: text("description").notNull(),
  impactMultiplier: numeric("impact_multiplier"),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const nftBonuses = pgTable("nft_bonuses", {
  id: serial("id").primaryKey(),
  gameTypeId: integer("game_type_id").references(() => gameTypes.id),
  traitType: varchar("trait_type").notNull(),
  traitValue: varchar("trait_value").notNull(),
  bonusType: varchar("bonus_type").notNull(), // 'stat', 'ability', 'cosmetic', 'economy'
  bonusValue: numeric("bonus_value").notNull(),
  description: text("description").notNull(),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
...
// User Cards Ownership
export const userCards = pgTable("user_cards", {
  id: serial("id").primaryKey(),
  walletAddress: text("wallet_address").notNull(),
  cardId: text("card_id").notNull(),
  cardName: text("card_name"),
  cardData: text("card_data"), // Store JSON string
  nftMint: text("nft_mint"), // Nullable, set when minted as cNFT
  source: text("source").notNull(), // 'starter', 'purchased', 'reward', 'nft'
...
// NFT Trait Analysis and Game Benefits
export const playerNftAnalysis = pgTable("player_nft_analysis", {
  id: serial("id").primaryKey(),
  wallet_address: text("wallet_address").notNull(),
  nft_mint: text("nft_mint").notNull(),
  nft_name: text("nft_name"),
  nft_image: text("nft_image"),
  nft_rank: integer("nft_rank"),
  traits: text("traits"), // JSON string of traits
  calculated_bonuses: text("calculated_bonuses"), // JSON string of bonuses
  deck_size: integer("deck_size"),
  captain_card_data: text("captain_card_data"), // JSON string of captain card
  last_updated: timestamp("last_updated").defaultNow().notNull(),
});

export const insertPlayerNftAnalysisSchema = createInsertSchema(playerNftAnalysis).pick({
  wallet_address: true,
  nft_mint: true,
  nft_name: true,
  nft_image: true,
  nft_rank: true,
  traits: true,
  calculated_bonuses: true,
  deck_size: true,
  captain_card_data: true,
});

export type InsertPlayerNftAnalysis = z.infer<typeof insertPlayerNftAnalysisSchema>;
export type PlayerNftAnalysis = typeof playerNftAnalysis.$inferSelect;
